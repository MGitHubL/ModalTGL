...
def train_val(args, train_val_data, model, mode, bs, epochs, criterion, optimizer, early_stopper, rand_samplers, logger, model_dim, n_hop=2):
  ...
  for epoch in range(epochs):
    ...
    modalTGL_results(logger, train_time, "train_time")
    ...
    # wandb.log({"ap": val_ap, "auc": val_auc})
    if epoch == 0:
...
def poly_loss(criterion, logits, labels):
    poly1 = torch.sum(labels * F.softmax(logits, dim=-1), dim=-1)
    ce_loss = criterion(logits, labels)
    poly1_ce_loss = ce_loss + (1 - poly1)

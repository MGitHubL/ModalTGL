...
for run in range(args.run):
  ...
  modalTGL.to(device)
  modalTGL.reset_store()

  optimizer = torch.optim.Adam(modalTGL.parameters(), lr=LEARNING_RATE)
  criterion = torch.nn.BCELoss()
  early_stopper = EarlyStopMonitor(tolerance=TOLERANCE)

  # start train and val phases
  train_val(args, train_val_data, modalTGL, args.mode, BATCH_SIZE, NUM_EPOCH, criterion, optimizer, early_stopper, rand_samplers, logger, model_dim, n_hop=NUM_HOP)

  # final testing
  print("_*"*50)
  if args.mode == 'i':
    modalTGL.reset_store()
    modalTGL.reset_self_rep()
    train_acc, train_ap, train_f1, train_auc = eval_one_epoch('test for {} nodes'.format(args.mode), modalTGL, all_train_val_rand_sampler, all_train_val_src_l, all_train_val_tgt_l, all_train_val_ts_l, all_train_val_label_l, all_train_val_e_idx_l, bs=32)
  test_start = time.time()
  test_acc, test_ap, test_f1, test_auc = eval_one_epoch('test for {} nodes'.format(args.mode), modalTGL, test_rand_sampler, test_src_l, test_tgt_l, test_ts_l, test_label_l, test_e_idx_l)
  test_end = time.time()
  ...
  total_end = time.time()
  print("ModalTGL experiment statistics:")
  if args.mode == "t":
    modalTGL_results(logger, transductive_ap, "transductive_ap")
    modalTGL_results(logger, transductive_auc, "transductive_auc")
    modalTGL_results(logger, transductive_acc, "transductive_acc")
  else:
    modalTGL_results(logger, inductive_ap, "inductive_ap")
    modalTGL_results(logger, inductive_auc, "inductive_auc")
    modalTGL_results(logger, inductive_acc, "inductive_acc")
    
  # modalTGL_results(logger, test_times, "test_times")
  # modalTGL_results(logger, early_stoppers, "early_stoppers")
  total_time.append(total_end - total_start)
  modalTGL_results(logger, total_time, "total_time")